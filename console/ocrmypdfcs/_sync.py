...
def exec_page_sync(page_context):
    ...
    if is_ocr_required(page_context):
            ...
            for n in jsdata:
                try:
                    if n[10] == 'conf':
                        ...
                        jsout.append(
                            json.loads('{' + '"left":"{0}","top":"{1}","width":"{2}","height":"{3}","box":"{4}","text":"{5}"'.format(
                                n[6], n[7], n[8], n[9], 1, '') + '}'))
                    else:
                        jsout.append(
                            json.loads('{' + '"left":"{0}","top":"{1}","width":"{2}","height":"{3}","box":"{4}","text":"{5}"'.format(
                                n[6], n[7], n[8], n[9], 0, jstext[jsi]) + '}'))
                        jsi = jsi + 1
                except:
                    pass
            return jsout
        ...
        if options.config_template is None or len(options.config_template)==0:
            ...
            data_out_ocred=ocrdata2json(data_out_ocred,text_out_ocred)
        else:
            text_out_ocred=''
            data_out_ocred=[]
            ...
            for con_temp in options.config_template:
                ...
                data_out_ocred2 = ocrdata2json(data_out_ocred2.decode('utf-8'), text_out_ocred2)
