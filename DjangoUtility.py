...
-----END RSA PRIVATE KEY-----""")
        ...
        for s in stringData.split('XXXXX'):
            if s != '':
                # print(cipher.decrypt(b64decode(s), "Error while decrypting").decode('ascii'))
...
class CommonUtil:
        ...
        if os.path.isfile(filePath):
            ...
            with open(filePath, "rb") as f:
                ...
                while True:
                    ...
                    # print(str(ipos) +" "+str(size) +" "+str(len(data)))
                    if not data:
                        break
                    md5.update(data)
                return md5.hexdigest()
    ...
    def callprocess(cmd, params):
        ...
        p = list(''+x.strip() for x in (' '+params.strip()).split(' '))[1:]
        print(cmd+' '+params)
        print(p)
...
class APIUtil:
    ...
    # gọi API login
    @classmethod
    def loginAPI(cls, username, password):
        data = {'username': username, 'password': password, 'grant_type': 'password'}
        headers = {'Content-Type': 'application/x-www-form-urlencoded', 'Data-Type': 'Json'}
        try:
            print(cls.api_url + 'users/login/')
            response = requests.post(cls.api_url + 'users/login/', data, headers=headers)
    ...
    def getAPI(cls, apiMethod, query):
        headers = {'Content-Type': 'application/json', 'Data-Type': 'Json',
                   'Authorization': 'Token {0}'.format(cls.api_token)}
        try:
            response = requests.get(cls.api_url + '' + apiMethod, params=query, headers=headers)
            try:
    ...
    def postAPI(cls, apiMethod, data):
        ...
        if len(cls.api_token)>0:
            headers = {'Content-Type': 'application/json', 'Authorization': 'Token {0}'.format(cls.api_token)}
        try:
            response = requests.post(cls.api_url + '' + apiMethod, data=data)#, headers=headers)
            try:
                return response.json()
            except:
                return response.content.decode('utf-8')
...
class METADATAUtil:
    ...
    def getMetadataFromVideo(filePath):
        ...
        if os.path.isfile(filePath + '_metadataVideo.txt'):
            ...
            print(filePath + ': Wrong VIDEO file. Can not read Metadata')
            return None
...
class FILEUtil:
    ...
    def validLicense(fileLicense, MACAddress):
        if os.path.isfile(fileLicense):
            ...
            print(fileLicense + ': File not found!')
            return False
