...
#xxxx
#from ...ocrmypdfcs.exceptions import EncryptedPdfError, MissingDependencyError
...
def _pdf_get_all_pageinfo(infile, detailed_analysis=False, log=None, progbar=False, page_number=''):
    ...
    for n, _ in tqdm(
        ...
        # Phongnh add page_number
        if page_number=='' or page_number==',,' or (page_number+'').find(','+str(n)+',')>=0:
...
class PdfInfo:
    ...
    # Phongnh add page_number
    def __init__(self, infile, detailed_page_analysis=False, log=logger, progbar=False, page_number=''):
        self._infile = infile
        self._pages, pdf = _pdf_get_all_pageinfo(
            # Phongnh add page_number
            infile, detailed_page_analysis, log=log, progbar=progbar, page_number=page_number
        )
