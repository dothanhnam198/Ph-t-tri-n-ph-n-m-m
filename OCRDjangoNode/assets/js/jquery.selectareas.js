    ...
    $.imageArea = function(parent, id) {
        var options = parent.options,
            ...
            updateBtDelete = function (visible) {
                if ($btDelete) {
                    ...
                    $btEdit.css({
                        display: visible ? "block" : "none",
                        position: "absolute",
                        left: area.x + area.width + 1 + 5,
                        top: area.y - $btEdit.outerHeight() - 1 + 53,
                        "z-index": area.z + 1
                    });
                }
            },
            ...
            changeTitle = function (event) {
                //cancelEvent(event);
                debugger
                area.title = $(event.target).closest('div').find('.i1').val();
                area.description = $(event.target).closest('div').find('.i2').val();
                fireEvent("changed");
            },
        ...
        if (options.allowDelete) {
            ...
            var bindToEdit = function ($obj) {
                $obj.bind("change", changeTitle);
                return $obj;
            };
            $btDelete = bindToDelete($("<div class=\"delete-area\" />"))
                .append(bindToDelete($("<div class=\"select-areas-delete-area\" />")))
                .insertAfter($selection);
            $btEdit = bindToEdit($("<div class=\"edit-area\" />"))
                .append(bindToEdit($("<input class=i1 placeholder=title title=title value=title><br>")))
                .append(bindToEdit($("<input class=i2 placeholder=description title=description >")))
