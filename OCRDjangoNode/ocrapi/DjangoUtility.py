...
class APIUtil:
    ...
    def loginAPI(cls, username, password, apiLoginMethod='users/login/', tokenname='auth_token'):
        ...
        try:
            ...
            response = requests.post(cls.api_url + apiLoginMethod, data, headers=headers)
    ...
    def getAPI(cls, apiMethod, query):
        headers = {'Content-Type': 'application/json', 'Data-Type': 'Json',
                   'Authorization': 'Token {0}'.format(cls.api_token), 'SITE-CODE': cls.siteapi_code}
        try:
            response = requests.get((cls.api_url + '' + apiMethod).replace('/api/fake_ftp/', '/api_fake_ftp/'), params=query, headers=headers)
            try:
    ...
    def postAPI(cls, apiMethod, data):
        headers = {'Content-Type': 'application/json', 'Authorization': 'Token {0}'.format(cls.api_token), 'SITE-CODE': cls.siteapi_code}
        try:
            response = requests.post((cls.api_url + '' + apiMethod).replace('/api/fake_ftp/', '/api_fake_ftp/'), data=data)#, headers=headers)
            try:
                return response.json()
            except:
                return response.content.decode('utf-8')
