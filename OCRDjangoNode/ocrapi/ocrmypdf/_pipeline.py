...
def get_pdfinfo(input_file, detailed_page_analysis=False, progbar=False, page_number=''):
    try:
        # Phongnh add page_number
        return PdfInfo(
            input_file, detailed_page_analysis=detailed_page_analysis, progbar=progbar, page_number=page_number
        )
...
def ocr_tesseract_data(input_image, page_context):
    output_pdf = page_context.get_path('ocr_tess.pdf')
    output_text = page_context.get_path('ocr_tess.data')
    options = page_context.options
    tesseract.generate_data(
        input_image=input_image,
        skip_pdf=None,
        output_pdf=output_pdf,
        output_text=output_text,
