...
def get_menu_items(context):
    ...
    # custom
    app_list_clone = []
    document = minidom.parse(app_settings.XML_DIR + '/module_tree.xml')
    appTags = document.getElementsByTagName('app')
    for appTag in appTags:
        if len(list(filter(lambda x: x['app_label'] == appTag.attributes['name'].nodeValue, app_list))) > 0:
            app = list(filter(lambda x: x['app_label'] == appTag.attributes['name'].nodeValue, app_list))[0]
            models_clone = []
            items_clone = []
            for child in appTag.childNodes:
                if child.nodeType == Element.ELEMENT_NODE and child.tagName == 'model':
                    if len(list(filter(lambda x: x['object_name'] == child.attributes['name'].nodeValue, app['models']))) > 0:
                        model = list(filter(lambda x: x['object_name'] == child.attributes['name'].nodeValue, app['models']))[0]
                        models_clone.append(model)

                        item = list(filter(lambda x: x['object_name'] == child.attributes['name'].nodeValue, app['items']))[0]
                        items_clone.append(item)
            app['models'] = models_clone
            app['items'] = items_clone
            app_list_clone.append(app)
    # end

    return app_list_clone
